...
## 8. Catalog 持久化迁移到 SQLite

### 8.1 动机

每一次评分、标记、备注修改都要把整份 Catalog 重新编码写盘。8,055 项时已是 6 MB，
5 万项约 37 MB——为改一个字段。

| 操作 | JSON 快照 | SQLite |
| --- | --- | --- |
| 改一次评分 | 全量重编码 + 写盘 | **单行 UPDATE，0.9 ms** |
| 插入 5 万行 | — | 0.071 s |
| 值没变的改动 | 仍然重写整表 | 不写盘 |

用系统自带的 `import SQLite3`，项目仍然零第三方依赖。并发靠一把锁而不是 SQLite
自己的串行模式，因为 `BEGIN`/`COMMIT` 之间不能被别的调用插进来。

### 8.2 写入从「快照」改为「操作」

`upsertSource` / `deleteSource` / `replaceAssets` / `updateAssets` / `deleteAssets`
各自只碰涉及的行。这也让原先"合并待写快照"的机制失去了存在理由——它唯一的目的
就是把多次昂贵的整表重写压成一次。

### 8.3 迁移策略

- 首次启动把 `catalog.json` 迁进数据库，随后把原文件**改名保留**为
  `catalog.json.pre-sqlite.bak`，而不是删除。它是这台机器上唯一一份迁移前的完整
  记录，迁移逻辑测得再多也不该在用户第一次启动新版本时就砍掉退路。
- 迁移只执行一次：把旧 JSON 放回原处也不会覆盖数据库里更新的数据。

### 8.4 真实数据演练

在真实 `catalog.json` 的副本上（不触碰原数据）：

```text
迁移 2 来源 / 1,874 资产：写入 0.039s，读回 0.002s
数据库 946 KB  vs  JSON 1.52 MB
逐字段比对：全部一致
```

比对是**逐字段**而不是只比数量——丢一个评分和丢一整张照片同样是数据损失。
按此外推，5 万项约 25 MB 数据库、迁移约 1 秒。

### 8.5 自动化回归

`swift build`、`swift test`（**147 tests / 37 suites**）、`Scripts/build-debug-app.sh`、
`git diff --check` 均 PASS。新增 7 项数据库测试，覆盖迁移无损（含 EditRecipe、OCR、
颜色标签）、备份保留、迁移不重复执行、删除来源级联删资产、重扫只替换指定来源、
空书签往返。

三个原本断言 `catalog.json` 文本内容的测试改为断言"重新打开后读得到"——
测试不该绑死在存储格式上。`CatalogWriter` 随它服务的快照路径一起移除。

## 9. 已知问题

### 9.1 从大图预览返回后网格空白，滚动即恢复

**现象**：首次打开「所有照片」正常；进入大图预览再返回，网格空白；**滚动一下即恢复**。

**根因**：库视图在覆盖层底下虽然仍在视图树中，但 LazyVGrid 会卸载它的格子；返回时
不会立即重建，要等一次滚动触发布局才重新实例化。

这也解释了为什么第 6 节里给 PhotoViewer 补的 `refreshVisibleSubscribers()` 没有解决
它：那个机制是给**已存在的格子**发信号，而这里格子根本不存在，信号没有接收方。
它对编辑器返回的场景仍然有效——那条路径下格子是被保留的。

**未修复的原因**：唯一直接的修法是给网格加 `.id` 强制重建，但那会把滚动位置弹回
顶部，比现状更烦人；要保住位置需要用 `scrollPosition` 做一套保存与恢复。

**将来重新评估的依据**：当初"保留库视图在树里"是因为重建格子会丢缩略图（当时只有
内存缓存）。现在派生图有了磁盘层，重建格子已经很便宜，这个取舍可以重新算一次。
