...
第五轮：Catalog 持久化迁移到 SQLite（已完成，详见报告第 8 节）

- [x] 新增 `CatalogDatabase`，用系统自带 `import SQLite3`，项目仍然零第三方依赖。
- [x] 写入从"整份快照"改为"按操作"：改一次评分是单行 UPDATE（实测 0.9 ms），
      不再全量重编码；值没变的改动不写盘。原先的快照合并机制随之移除。
- [x] 首次启动自动迁移 `catalog.json`，原文件改名保留为 `.pre-sqlite.bak` 而非删除；
      迁移只执行一次，旧 JSON 放回原处也不会覆盖更新的数据。
- [x] 真实数据演练：2 来源 / 1,874 资产，写入 0.039s、读回 0.002s，**逐字段比对全部一致**。
- [x] `swift build`、`swift test`（147 tests / 37 suites）、`Scripts/build-debug-app.sh`、
      `git diff --check` 均 PASS。
- [ ] 真机验证：首次启动的实际迁移结果（照片数、评分、标记）。

已知问题（见报告第 9 节）：

- [ ] 从大图预览返回后网格空白，**滚动即恢复**。根因是 LazyVGrid 在覆盖层底下卸载了
      格子、返回时不重建；`refreshVisibleSubscribers()` 对它无效，因为那是给已存在的
      格子发信号。直接修法会弹回滚动顶部，需配合 `scrollPosition` 保存恢复。
      用户已确认影响可接受，暂不修复。
